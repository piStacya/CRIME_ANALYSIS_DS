...
const getCountryValue = (crimeCode, year) => {
  ...
  return crimeTypeData.per100k?.[props.countryCode] ?? null
}
...
const heatmapData = computed(() => {
  ...
  const crimes = crimesMetadata.value.filter(crime => {
    let hasData = false
    for (const year of years) {
      if (getCountryValue(crime.code, year) !== null) {
        hasData = true
        break
      }
    }
  ...
  for (const crime of crimes) {
    ...
    for (let i = 0; i < years.length; i++) {
      const year = years[i]
      const currValue = getCountryValue(crime.code, year)

      if (i === 0) {
        row.push(currValue !== null ? 1 : null)
        valueRow.push({ prev: null, curr: currValue })
      } else {
        const prevYear = years[i - 1]
        const prevValue = getCountryValue(crime.code, prevYear)

        if (currValue !== null && prevValue !== null && prevValue > 0) {
          const ratio = currValue / prevValue
          row.push(ratio)
          valueRow.push({ prev: prevValue, curr: currValue })
        } else {
          row.push(null)
          valueRow.push({ prev: prevValue, curr: currValue })
        }
...
<template>
  <div class="heatmap-container">
    ...
    <div v-else-if="heatmapData.crimes.length === 0" class="heatmap-loading">
      <p>{{ t('common.noData') }}</p>
    </div>
      ...
      <Teleport to="body">
        ...
        >
          ...
          <div v-if="tooltip.prevValue !== null" class="tooltip-details">
            {{ tooltip.prevValue?.toFixed(1) }} → {{ tooltip.currValue?.toFixed(1) }} {{ t('common.per100k') }}
          </div>
        </div>
      </Teleport>
    </div>
